  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    const fireLoop = () => {
      confetti({
        ...
        colors: ['#ff0000', '#ff4500', '#ff8c00', '#ffd700', '#ffffff', '#111111'],
    ...
    >
        ...
        <svg className="absolute w-0 h-0 pointer-events-none">
          <filter id="tear-effect">
            <feTurbulence type="fractalNoise" baseFrequency="0.03" numOctaves="3" result="noise">
              <animate attributeName="baseFrequency" values="0.03; 0.15" dur="2.5s" fill="freeze" />
            </feTurbulence>
            <feDisplacementMap in="SourceGraphic" in2="noise" scale="0" xChannelSelector="R" yChannelSelector="G">
              <animate attributeName="scale" values="0; 150" dur="2.5s" fill="freeze" />
            </feDisplacementMap>
          </filter>
        </svg>
      )}
      ...
      <AnimatePresence mode="wait">
        {!clicked ? (
          ...
          <motion.div
            key="seal-burn"
            initial={{ scale: 1, filter: 'url(#tear-effect) brightness(1) contrast(1)', opacity: 1 }}
            animate={{
              scale: 0.9,
            ...
            style={{ boxShadow: '0 0 80px 20px rgba(255,80,0,0.9), inset 0 0 20px 10px rgba(255,200,0,0.8)' }}
          >
