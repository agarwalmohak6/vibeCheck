...
const TEMPLATE_SAMPLES: Record<string, { emoji: string; badge: string; title: string; body: string; yesMsg: string }> = {
  shoot_shot: {
    emoji: '💍',
...
export default function LiveSimulator() {
  ...
  const sample = TEMPLATE_SAMPLES[activeTemplate] || TEMPLATE_SAMPLES.shoot_shot;
  ...
  const handleYes = () => {
    ...
    setTimeout(() => confetti({ particleCount: 100, spread: 90, origin: { y: 0.6, x: 0.3 }, colors }), 300);
    setTimeout(() => confetti({ particleCount: 100, spread: 90, origin: { y: 0.6, x: 0.7 }, colors }), 500);
  };
    ...
    <div className="flex flex-col items-center gap-5">
      ...
      <div className="flex flex-wrap justify-center gap-2 max-w-sm">
        {TEMPLATE_TYPES.map((t) => (
          <motion.button
            ...
            style={{
              background: activeTemplate === t.id ? 'linear-gradient(135deg, var(--accent), var(--accent2))' : 'var(--surface)',
              color: activeTemplate === t.id ? 'white' : 'var(--text3)',
              border: activeTemplate === t.id ? 'none' : '1px solid var(--border)',
              boxShadow: activeTemplate === t.id ? '0 4px 20px var(--glow)' : 'none',
      ...
      >
        ...
        <motion.div
          ...
          animate={{ boxShadow: ['0 0 30px var(--glow)', '0 0 60px var(--glow)', '0 0 30px var(--glow)'] }}
        ...
        <div
          ...
          style={{
            background: 'var(--bg)',
            borderColor: '#1a1a2e',
            boxShadow: '0 35px 90px rgba(0,0,0,0.7), inset 0 0 80px rgba(0,0,0,0.3)',
          }}
        >
          {/* Status bar */}
          <div className="flex justify-between items-center px-5 pt-3 pb-1 z-20 relative" style={{ color: 'var(--text3)' }}>
          ...
          {/* Screen content */}
          <div className="relative h-full overflow-hidden" style={{ paddingTop: 6 }}>
            <AnimatePresence mode="wait">
              ...
              {!envelopeOpen && (
                ...
                >
                  ...
                  <p className="text-center text-xs font-medium" style={{ color: 'var(--text3)' }}>
                  ...
                  >
                    {isOpening ? '✨ Opening...' : (theme === 'desi_festive' || theme === 'bollywood_drama') ? '✨ Tap Wax Seal to Break' : '✨ Tap to open'}
                  </motion.p>
                </motion.div>
              )}
              ...
              {envelopeOpen && !yesClicked && (
                ...
                >
                  ...
                  >
                    <p className="text-[10px] mb-1.5 font-black text-center" style={{ color: 'var(--text3)' }}>
                      So... what do you say? 👀
                    </p>
                    <p className="text-[9px] mb-2 text-center" style={{ color: 'var(--text3)', opacity: 0.7 }}>
                  ...
                  <motion.button
                    ...
                    style={{ background: 'var(--surface2)', color: 'var(--text3)', flexShrink: 0 }}
                  >
                    ↩ Reset demo
                  </motion.button>
                </motion.div>
      ...
      <p className="text-[10px] font-semibold text-center max-w-[260px] leading-relaxed" style={{ color: 'var(--text3)' }}>
