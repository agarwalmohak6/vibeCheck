...
export default function RunawayButton({
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const cx = rect.left + rect.width / 2;
    const cy = rect.top + rect.height / 2;
    if (Math.hypot(e.touches[0].clientX - cx, e.touches[0].clientY - cy) < 65)
      dodge();
  };
  ...
  return (
    ...
    >
          ...
          animate={
            yesClicked
              ...
              : {
                  scale: noGone
                    ? [1.6, 1.7, 1.6]
                    : Math.min(1.6, 1 + dodgeCount * 0.15),
                  boxShadow: noGone
                    ? [
                        "0 0 20px var(--glow)",
                        "0 0 40px var(--glow)",
        ...
        {farewellMsg && (
          ...
          >
            <span
              ...
              style={{
                background: farewellMsg.color + "22",
                color: farewellMsg.color,
                border: `1px solid ${farewellMsg.color}44`,
              }}
            >
              {farewellMsg.text}
            </span>
      ...
      <AnimatePresence>
        {showRejection && (
          <MemeRejection onClose={() => setShowRejection(false)} />
        )}
      </AnimatePresence>
    </div>
  );
